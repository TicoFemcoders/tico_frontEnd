...
const StatCards = ({ stats }) => {
  return (
    <Box sx={{ display: 'flex', gap:3, width: '100%', mb: 4,  flexDirection: { xs: 'column', sm: 'row' }  }}>
        {stats.map((stat) => (
            <Card sx={{ p: 2, borderLeft: `6px solid`, borderColor: stat.color, borderRadius: 2, boxShadow: 1, width: '100%',  display: 'flex',
                ...
                <Typography variant="h1" sx={{ fontSize: "2.5rem", mt: 1 }}>{stat.value}</Typography>
            </Card>
        ))}
        </Box>
  );
